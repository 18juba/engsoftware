<script lang="ts">
  ...
  function getHandler(key: ActionKey): () => void {
    switch (key) {
      case 'iniciar': return iniciarTarefa;
      case 'pausar': return pausarTarefa;
      case 'concluir': return concluirTarefa;
      case 'cancelar': return cancelarTarefa;
      default: return () => {};
    }
  }
      ...
      <div class="p-1">
        {#each acoes as acao (acao.key)}
          <button
            ...
            disabled={isLoading}
            class="flex w-full items-center gap-2 px-4 py-2 text-left text-sm text-(--foreground) transition-colors hover:bg-(--foreground)/10 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
